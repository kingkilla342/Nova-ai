...
export default function Builder({ projectId }) {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden grid-bg">
      {/* TOP BAR */}
      <header className="flex-shrink-0 hud-panel-strong border-b border-[rgba(0,255,106,0.15)] h-12 px-4 flex items-center justify-between relative z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="w-6 h-7 hexagon bg-[rgba(0,255,106,0.1)] flex items-center justify-center">
            <span className="font-display font-bold text-hud-green text-[14px]">N</span>
          </div>
          <span className="font-display font-semibold text-hud-green text-glow-subtle text-base tracking-[0.15em]">{project.name.toUpperCase()}</span>
          <span className="font-mono text-[12px] text-hud-text-dim px-1.5 py-0.5 rounded border border-[rgba(0,255,106,0.1)] bg-[rgba(0,255,106,0.03)]">
            v{project.version}
          </span>
          <span className="font-mono text-[12px] text-hud-cyan px-1.5 py-0.5 rounded border border-[rgba(0,221,255,0.15)] bg-[rgba(0,221,255,0.05)]">
            MC {project.mcVersion || '1.20.4'}
          </span>
          <div className="flex items-center gap-1.5">
            <div className="status-dot" />
            <span className="font-mono text-[12px] text-hud-green">READY</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {['ai-focus', 'balanced', 'code-focus'].map(l => (
            <button
              key={l}
              onClick={() => setLayout(l)}
              className={`px-2.5 py-1 rounded text-[12px] font-display font-semibold tracking-wider transition-all border ${
                layout === l
          ...
          <button onClick={() => setTerminalOpen(!terminalOpen)} className={`px-2 py-1 rounded text-[12px] font-display tracking-wider transition-all border ${terminalOpen ? 'bg-[rgba(0,255,106,0.08)] border-[rgba(0,255,106,0.25)] text-hud-green' : 'border-transparent text-hud-text-dim'}`}>
            TERM
          </button>
          <button
            onClick={() => runValidation(files)}
            className="px-2 py-1 rounded text-[12px] font-display tracking-wider border border-transparent text-hud-amber hover:border-[rgba(255,170,0,0.25)] hover:bg-[rgba(255,170,0,0.05)] transition"
          ...
          <button
            ...
            className="px-2 py-1 rounded text-[12px] font-display tracking-wider border border-transparent text-hud-cyan hover:border-[rgba(0,221,255,0.25)] hover:bg-[rgba(0,221,255,0.05)] transition"
          ...
          <button
            ...
            className="px-2 py-1 rounded text-[12px] font-display tracking-wider border border-transparent text-hud-text-bright hover:border-[rgba(0,255,106,0.25)] hover:bg-[rgba(0,255,106,0.05)] transition"
          ...
          <button
            ...
            className="px-2 py-1 rounded text-[12px] font-display tracking-wider border border-transparent text-hud-green hover:border-[rgba(0,255,106,0.25)] hover:bg-[rgba(0,255,106,0.05)] transition"
      ...
      {showCompile && <CompilePanel project={project} files={files} onClose={() => setShowCompile(false)} />}
    </div>
  );
}
