  ...
  loading: () => (
    <div className="h-full flex items-center justify-center">
      <div className="flex items-center gap-2">
        ...
        <span className="font-mono text-[12px] text-hud-text-dim tracking-wider">LOADING EDITOR...</span>
      </div>
    </div>
  ),
});
...
export default function CodeEditor({ files, activeFile, onUpdate }) {
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: '#020a04' }}>
      {/* Tab bar */}
      <div className="flex-shrink-0 h-9 border-b border-[rgba(0,255,106,0.1)] flex items-center px-2 hud-panel relative z-10">
        {activeFile ? (
          <div className="flex items-center gap-2 px-2 py-1 rounded border border-[rgba(0,255,106,0.15)] bg-[rgba(0,255,106,0.04)]">
            ...
            <span className="font-mono text-[13px] text-hud-green tracking-wider">{getFileName(activeFile)}</span>
            <span className="font-mono text-[13px] text-hud-text-dim uppercase tracking-widest">{language}</span>
          </div>
        ) : (
          <span className="font-mono text-[12px] text-hud-text-dim tracking-wider">NO FILE SELECTED</span>
        )}
      </div>
        ...
        {activeFile ? (
          ...
          <div className="h-full flex items-center justify-center">
            <div className="text-center animate-fade">
              ...
              <p className="font-mono text-[12px] text-hud-text-dim tracking-wider">SELECT A FILE TO EDIT</p>
            </div>
          </div>
        )}
      </div>
