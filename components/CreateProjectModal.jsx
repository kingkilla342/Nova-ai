    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 animate-fade">
      <div className="hud-panel-strong hud-corners hud-corners-bottom rounded-lg w-full max-w-lg mx-4 overflow-hidden animate-materialize">
        ...
        <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          {/* Name */}
          <div>
            <label className="block font-mono text-[9px] text-hud-text-dim tracking-[0.3em] uppercase mb-1.5">PROJECT DESIGNATION</label>
            <input
          ...
          {/* Type */}
          <div>
            <label className="block font-mono text-[9px] text-hud-text-dim tracking-[0.3em] uppercase mb-1.5">PROJECT TYPE</label>
            <div className="grid grid-cols-3 gap-2">
              {Object.entries(PROJECT_TYPES).map(([key, { label }]) => (
                ...
                >
                  <div className="font-display text-[11px] font-bold tracking-wider">{key.toUpperCase()}</div>
                  <div className="font-mono text-[8px] mt-0.5 opacity-60">{label.match(/\((.+)\)/)?.[1] || label}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Minecraft Version */}
          <div>
            <label className="block font-mono text-[9px] text-hud-text-dim tracking-[0.3em] uppercase mb-1.5">MINECRAFT VERSION</label>
            <div className="grid grid-cols-4 gap-1.5 max-h-28 overflow-y-auto pr-1">
              {MC_VERSIONS.map(v => (
                ...
                >
                  <div className="font-mono text-[11px] font-semibold">{v.value}</div>
                </button>
              ))}
            </div>
            {/* Version info */}
            {selectedMC && (
              <div className="mt-2 flex gap-3 font-mono text-[8px] text-hud-text-dim tracking-wider">
          ...
          {type !== 'datapack' && (
            <div>
              <label className="block font-mono text-[9px] text-hud-text-dim tracking-[0.3em] uppercase mb-1.5">PACKAGE NAMESPACE</label>
              <input
          ...
          {/* Version */}
          <div>
            <label className="block font-mono text-[9px] text-hud-text-dim tracking-[0.3em] uppercase mb-1.5">PLUGIN VERSION</label>
            <input
