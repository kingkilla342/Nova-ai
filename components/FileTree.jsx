  ...
  if (!node.isDir) {
    ...
    return (
      ...
      >
        ...
        <span className="font-mono text-[13px] truncate flex-1">{node.name}</span>
        <span
          onClick={(e) => { e.stopPropagation(); onDelete(node.path); }}
          className="hidden group-hover:flex w-4 h-4 items-center justify-center text-hud-text-dim hover:text-hud-red text-[12px] cursor-pointer rounded hover:bg-[rgba(255,51,68,0.1)]"
        >×</span>
      </button>
    );
  }
    ...
    <div>
      {node.name && (
        ...
        >
          ...
          <span className="font-mono text-[13px] font-semibold tracking-wider">{node.name}</span>
          <span className="font-mono text-[13px] text-hud-text-dim opacity-50 ml-auto">{Object.keys(node.children).length}</span>
        </button>
      )}
...
export default function FileTree({ files, activeFile, onSelect, onCreate, onDelete }) {
  ...
  return (
    <div className="h-full flex flex-col scan-line">
      <div className="flex-shrink-0 px-4 py-2.5 border-b border-[rgba(0,255,106,0.1)] flex items-center justify-between hud-panel relative z-10">
        <div className="flex items-center gap-2">
          <span className="font-display text-[13px] text-hud-green font-semibold tracking-[0.2em]">FILES</span>
          <span className="font-mono text-[14px] text-hud-text-dim px-1 py-0.5 rounded border border-[rgba(0,255,106,0.1)] bg-[rgba(0,255,106,0.03)]">{fileCount}</span>
        </div>
      ...
      {showInput && (
        <div className="px-2 py-2 border-b border-[rgba(0,255,106,0.08)]">
          <input
            ...
            className="w-full px-2 py-1.5 rounded hud-input text-[13px]"
            autoFocus
          />
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto py-1">
        {fileCount === 0 ? (
          <p className="font-mono text-[12px] text-hud-text-dim px-3 py-4 text-center tracking-wider">NO FILES DETECTED</p>
        ) : (
