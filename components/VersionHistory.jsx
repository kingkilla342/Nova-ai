      ...
      <div className="hud-panel-strong hud-corners hud-corners-bottom rounded-lg w-full max-w-md mx-4 overflow-hidden animate-materialize">
        ...
        <div className="px-5 pt-4 pb-3 border-b border-[rgba(0,255,106,0.1)] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display font-bold text-hud-green tracking-[0.2em] text-sm">VERSION HISTORY</h2>
          </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto p-4 space-y-2">
          {versions.length === 0 ? (
            <div className="text-center py-8">
              <p className="font-mono text-[10px] text-hud-text-dim tracking-wider">NO SNAPSHOTS YET</p>
            </div>
          ) : (
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded flex items-center justify-center font-display text-[9px] font-bold ${
                      i === 0 ? 'bg-[rgba(0,255,106,0.15)] text-hud-green' : 'bg-[rgba(0,255,106,0.05)] text-hud-text-dim'
                    }`}>
                      v{v.number}
                    </div>
                    <div>
                      <div className="font-sans text-xs text-hud-text-bright">{v.label}</div>
                      <div className="font-mono text-[8px] text-hud-text-dim tracking-wider">{formatTime(v.timestamp)}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {i !== 0 && (
                      <button
                        ...
                        className="hud-btn px-2 py-1 rounded text-[8px] tracking-wider"
                      >
                        RESTORE
                      </button>
                    )}
                    {i === 0 && (
                      <span className="font-mono text-[8px] text-hud-green tracking-wider px-2 py-1 rounded bg-[rgba(0,255,106,0.08)] border border-[rgba(0,255,106,0.2)]">
                        CURRENT
                      </span>
                    )}
                  </div>
        ...
        <div className="px-5 py-3 border-t border-[rgba(0,255,106,0.1)] flex items-center justify-between">
          <span className="font-mono text-[8px] text-hud-text-dim tracking-wider">{versions.length} SNAPSHOT{versions.length !== 1 ? 'S' : ''}</span>
          <button onClick={onClose} className="hud-btn px-4 py-1.5 rounded text-[10px] tracking-wider">CLOSE</button>
        </div>
      </div>
      ...
      {confirmRollback && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] animate-fade">
          <div className="hud-panel-strong rounded-lg p-5 w-full max-w-xs mx-4 animate-materialize">
            <h3 className="font-display text-hud-amber text-sm tracking-wider mb-2">⚠ CONFIRM ROLLBACK</h3>
            <p className="font-mono text-[10px] text-hud-text-dim mb-4 leading-relaxed">
              This will replace all current files with the snapshot. Current state will be saved as a new snapshot first.
            </p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setConfirmRollback(null)} className="hud-btn px-3 py-1.5 rounded text-[10px] tracking-wider">CANCEL</button>
              <button
                onClick={() => handleRollback(confirmRollback)}
                className="px-3 py-1.5 rounded text-[10px] tracking-wider font-semibold bg-hud-amber/20 border border-hud-amber/40 text-hud-amber hover:bg-hud-amber/30 transition"
              >
                ROLLBACK
              </button>
            </div>
